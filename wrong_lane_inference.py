...
#frame and boxes to the data
 ...
 #   print(results)
   ...
   #  x1,y1 and x2,y2 are the two coordinate points        
...
#        print(row) 
        ...
        x2=int(row[2])
        y2=int(row[3])
        #same cars detection id that same cars the passing blue line red line
        d=int(row[5])
    ...
    for bbox in bbox_id:
        ...
        ## range value buffer value kept in offset
        if red_line_y < (cy + offset) and red_line_y > (cy - offset):
          down[id]=cy   
        if id in down:
           if blue_line_y < (cy + offset) and blue_line_y > (cy - offset):         
